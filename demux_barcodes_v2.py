...
def main(fastq_files, barcode_csv, outdir, min_length, max_penalty, cpus, flank):
    """
    fastq_files: either a single FASTQ path (string) or a list of FASTQ paths.
    """
    # Accept both a string or a list
    if isinstance(fastq_files, str):
        fastq_files = [fastq_files]

    os.makedirs(outdir, exist_ok=True)

    # Load the barcodes once
    row_map, col_map = load_barcodes(barcode_csv)

    for fastq_file in fastq_files:
        group = os.path.splitext(os.path.basename(fastq_file))[0]  # "group1"
        group_outdir = os.path.join(outdir, group)
        os.makedirs(group_outdir, exist_ok=True)

        print(f"\n=== Processing group: {group} ===")
        print(f"FASTQ: {fastq_file}")
        print(f"Output → {group_outdir}")

        # Build jobs list from chunks for this FASTQ
        jobs = [(chunk, row_map, col_map, min_length, max_penalty, flank)
                for chunk in chunk_reads(fastq_file)]

        if not jobs:
            print(f"⚠️ No reads or failed to read {fastq_file} — skipping.")
            continue

        # Use a context manager so the pool is properly closed on Windows/macOS/Linux
        ...
        for well, reads in all_reads.items():
            out_fastq = os.path.join(group_outdir, f"{well}_reads.fastq")
            SeqIO.write(reads, out_fastq, "fastq")
            
            # FASTA
            out_fasta = os.path.join(group_outdir, f"{well}_reads.fasta")
            SeqIO.write(reads, out_fasta, "fasta")
            
        # Write barcode stats
        stats_path = os.path.join(group_outdir, f"barcode_stats_{group}.csv")
        write_barcode_grid_csv(all_stats, stats_path)

        print(f"✔ Finished {group}: stats written to {stats_path}")
